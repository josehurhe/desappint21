...
namespace p18_repasopoov2
{
    class Program
    {
        ...
        {
            Red mired = null;
            string archivo = "datos.xml";
            string archivoJ = "datos.json";

            if (File.Exists(archivoJ)){
                // cargar los datos del archivo que ya existe en disco: datos.xml
                Console.WriteLine($"Cargando los datos del archivo de disco : {archivo}");
                //Utils.LeerXml(archivo,ref mired);
                Utils.LeerJson(archivoJ,ref mired);
            } else{
                ...
                //Utils.GrabarXml(archivo,mired);
                Utils.GrabarJson(archivoJ,mired);
            }
            Reporte(mired);
        }
        ...
        static void Inicializa(ref Red mired){
            // Inicializar datos
            mired = new Red("Universidad Autonoma de Zacatecas","Juan Perez","Jardín Juarez 147");
